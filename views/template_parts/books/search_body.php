<?php if (sizeof($bodyData['data']) == 0): ?>
    <h2>По запросу <b>"<?= $bodyData['searchQuery'] ?>"</b> ничего не найдено</h2>
<?php else: ?>
    <h2>Результаты поиска по запросу <b>"<?= $bodyData['searchQuery'] ?>"</b>:</h2>
<?php endif; ?>
<?php foreach ($bodyData['data'] as $record): ?>
    <div data-book-id="<?= $record['book_id'] ?>" class="book_item col-xs-6 col-sm-3 col-md-2 col-lg-2">
        <div class="book">
            <a href="book/<?= $record['book_id'] ?>"><img src="<?= '/uploads/' . $record['cover'] ?>"
                                                          alt="<?= $record['title'] ?>">
